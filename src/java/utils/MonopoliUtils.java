...
public class MonopoliUtils {
    ...
    private void controlCasillaNormal(Jugador jugador, Partida partida, int newPosicion) {
        ...
        switch (jugador.getEstado()) {
            case ESTADO_PAGAR:
                float precio = calcularPrecio(newPosicion);
                jugador.setDinero(jugador.getDinero() - precio);
                propietario.setDinero(propietario.getDinero() + precio);
                jugador.setInfoPlayer(
                        "Acabas de pagar al propietario " + propietario.getNick() + " " + precio + " euros por caer en su casilla");
                propietario.setInfoPlayer("Acabas de cobrar del jugador " + jugador.getNick()
                        + " " + precio + " euros por que ha caido en una casilla tuya");
                break;
            case ESTADO_COMPRAR:
    ...
    public float calcularPrecio(int posicion, int numCasas) {
        ...
        float precio = precio(posicion)*numCasas;
        return precio;
